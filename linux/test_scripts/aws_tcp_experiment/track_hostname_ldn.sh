...
if grep -q "^$CURRENT_HOSTNAME," hostnames.txt; then
    ...
    if [[ "$CURRENT_HOSTNAME" == *"customer.lndngbr"* && $TIME_DIFF -ge 300 && $TIME_DIFF -le 5400 && "$WAS_EXECUTED" != "1" ]]; then
        echo "Detected target hostname after 5 minutes - starting aws experiment to eu-west-2"
        # Update entry with execution flag
        grep -v "^$CURRENT_HOSTNAME," hostnames.txt > hostnames.tmp
        echo "$CURRENT_HOSTNAME,$CURRENT_EPOCH,1" >> hostnames.tmp
        mv hostnames.tmp hostnames.txt
        ...
        myprint "Executing: run-aws-client.sh -s eu-west-2 --dur $DUR -r $RATE --tcpdump -c vegas -e $CURRENT_EPOCH --ID eu-west-2"
        ./run-aws-client.sh -s eu-west-2 --dur $DUR -r $RATE --tcpdump -c vegas -e "$CURRENT_EPOCH" --ID eu-west-2
    ...
    # Frankfurt PoP: 800-3000 seconds after first encounter 
    if [[ "$CURRENT_HOSTNAME" == *"customer.frntdeu"* && $TIME_DIFF -ge 800 && $TIME_DIFF -le 3000 && "$WAS_EXECUTED" != "1" ]]; then
        echo "Detected target hostname after 800-3000 seconds - starting aws experiment to eu-central-1"
        # Update entry with execution flag
        grep -v "^$CURRENT_HOSTNAME," hostnames.txt > hostnames.tmp
        echo "$CURRENT_HOSTNAME,$CURRENT_EPOCH,1" >> hostnames.tmp
        mv hostnames.tmp hostnames.txt
        ...
        myprint "Executing: run-aws-client.sh -s eu-central-1 --dur $DUR -r $RATE --tcpdump -c vegas -e $CURRENT_EPOCH --ID eu-central-1"
        ./run-aws-client.sh -s eu-central-1 --dur $DUR -r $RATE --tcpdump -c vegas -e "$CURRENT_EPOCH" --ID eu-central-1
        myprint "Executing: run-aws-client.sh -s eu-west-2 --dur $DUR -r $RATE --tcpdump -e $CURRENT_EPOCH --ID eu-west-2"
        ./run-aws-client.sh -s eu-west-2 --dur $DUR -r $RATE --tcpdump -e "$CURRENT_EPOCH" --ID eu-west-2
    ...
    if [[ "$CURRENT_HOSTNAME" == *"customer.mlnnita1"* && $TIME_DIFF -ge 900 && $TIME_DIFF -le 3600 && "$WAS_EXECUTED" != "1" ]]; then
        ...
        myprint "Executing: run-aws-client.sh -s eu-south-1 --dur $DUR -r $RATE --tcpdump -c vegas -e $CURRENT_EPOCH --ID eu-south-1"
        ./run-aws-client.sh -s eu-south-1 --dur $DUR -r $RATE --tcpdump -c vegas -e "$CURRENT_EPOCH" --ID eu-south-1
        myprint "Executing: run-aws-client.sh -s eu-west-2 --dur $DUR -r $RATE --tcpdump -e $CURRENT_EPOCH --ID eu-west-2"
        ./run-aws-client.sh -s eu-west-2 --dur $DUR -r $RATE --tcpdump -e "$CURRENT_EPOCH" --ID eu-west-2
    ...
    if [[ "$CURRENT_HOSTNAME" == *"customer.mdrdesp1"* && $TIME_DIFF -ge 600 && $TIME_DIFF -le 3600 && "$WAS_EXECUTED" != "1" ]]; then
        ...
        myprint "Executing: run-aws-client.sh -s eu-south-2 --dur $DUR -r $RATE --tcpdump -c vegas -e $CURRENT_EPOCH --ID eu-south-2"
        ./run-aws-client.sh -s eu-south-2 --dur $DUR -r $RATE --tcpdump -c vegas -e "$CURRENT_EPOCH" --ID eu-south-2
        myprint "Executing: run-aws-client.sh -s eu-west-2 --dur $DUR -r $RATE --tcpdump -e $CURRENT_EPOCH --ID eu-west-2"
        ./run-aws-client.sh -s eu-west-2 --dur $DUR -r $RATE --tcpdump -e "$CURRENT_EPOCH" --ID eu-west-2
    ...
    # Doha PoP: 10-60 minutes after first encounter 
    if [[ "$CURRENT_HOSTNAME" == *"customer.dohaqat"* && $TIME_DIFF -ge 600 && $TIME_DIFF -le 3600 && "$WAS_EXECUTED" != "1" ]]; then
        echo "Detected target hostname after 10-60 minutes - starting aws experiment to me-central-1"
        # Update entry with execution flag
        grep -v "^$CURRENT_HOSTNAME," hostnames.txt > hostnames.tmp
        echo "$CURRENT_HOSTNAME,$CURRENT_EPOCH,1" >> hostnames.tmp
        mv hostnames.tmp hostnames.txt
        ...
        myprint "Executing: run-aws-client.sh -s me-central-1 --dur $DUR -r $RATE --tcpdump -c vegas -e $CURRENT_EPOCH --ID me-central-1"
        ./run-aws-client.sh -s me-central-1 --dur $DUR -r $RATE --tcpdump -c vegas -e "$CURRENT_EPOCH" --ID me-central-1
        myprint "Executing: run-aws-client.sh -s eu-west-2 --dur $DUR -r $RATE --tcpdump -e $CURRENT_EPOCH --ID eu-west-2"
        ./run-aws-client.sh -s eu-west-2 --dur $DUR -r $RATE --tcpdump -e "$CURRENT_EPOCH" --ID eu-west-2
